...
const to_iso_string = (src: string) => moment(src).toISOString()
...
export default {
    ...
    // 轉時間格式
    iso_date_for_form(src: ONE, k: string[]) {
        for (const i in k) { const _k = k[i]; if (src[_k]) { src[_k] = to_iso_string(src[_k]) } }
        return src
    }
}