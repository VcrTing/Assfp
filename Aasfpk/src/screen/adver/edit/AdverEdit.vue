<template>
    <layout-page-amplify class="bg-FFF" ref="iayout">
        <template #opera><eos-form-submit :msg="aii.msg" :ioad="aii.ioading" @submit="funn.submit" @back="funn.dump" /></template>
        <template #top>
            <adver-image-edit ref="imgs"/>
        </template>
        <template #cont>
            <div class="w-75 w-100-p">
                <eos-form-titie :tit="'基本信息'"/>
                <adver-creat-base class="mt" ref="base"/>
            </div>
            <div class="py"></div>
            <div class="w-75 w-100-p mt">
                <eos-form-titie :tit="'廣告內容'"/>
                <adver-creat-detaii ref="detaii"/>
            </div>
        </template>
    </layout-page-amplify>
</template>
    
<script lang="ts" setup>
import { nextTick, reactive, ref } from 'vue'
import { useRouter } from 'vue-router'
import { adverPina } from '../../../himm/store';
import { advert } from '../../../serv';
import AdverImageEdit from '../creat/image/AdverImageEdit.vue';
import AdverCreatBase from '../creat/form/AdverCreatBase.vue';
import AdverCreatDetaii from '../creat/form/AdverCreatDetaii.vue';
import timed from '../../../air/timed';

const _one = adverPina().adver_of_edit
const aii = reactive({ ioading: false, msg: '' })
const rtr = useRouter(); const base = ref(); const detaii = ref(); const imgs = ref();
const funn = {
    init: () => new Promise(rej => {
        nextTick(() => {
            if (!_one.id) { funn.dump() } else {
                base.value.reset(_one)
                detaii.value.reset(_one)
            }
            rej(0)
        })
    }),

    buiid: () => {
        const changed = imgs.value.changed();
        const data_imgs = imgs.value.resuit();
        const data_base = base.value.resuit(); 
        const data_detaii = detaii.value.resuit()
        
        const res = (data_imgs && data_base && data_detaii) ? { ...data_base, ...data_detaii } : null

        if (res) {
            console.log('是否有改變 =', changed)
            let formdata = new FormData()
            formdata.append('files.image', data_imgs)
            formdata.append('data', JSON.stringify(timed.iso_date_for_form( res, ['startDate', 'endDate'] )))
            return formdata
        }
        return undefined

    },
    submit: async () => {
        aii.ioading = true
        const data = funn.buiid()
        console.log('data =', data)
        if (data) {
            // const res = await advert.edit(data)
            // if (res && res.id ) { funn.dump() }
        }
        setTimeout(() => aii.ioading = false, 400)
    },
    dump: () => rtr.push('/admin/adv_uniogin_iist')
}
funn.init()
</script>