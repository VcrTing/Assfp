...
const funn = {
    init: () => new Promise(rej => {
        ...
        nextTick(() => { aii.y = timed.y(); aii.m = funn.gen_m_end(); rej(0) })
    }),
    gen_m_end: () => { let end = timed.m() + 1; return end >= 12 ? 12 : end; },
    gen_ms: () => { if (aii.y == timed.y()) { aii.ms = timed.month( funn.gen_m_end() );  } else { aii.ms = timed.month(12) } },
    to: (n: number) => {
        let pre = aii.m + n
