...
const many = async (params: ONE): Promise<ONE> => {
    let res = await net.get('notific', userPina().jwt, params)
    return res ? strapi.ser_aii(res, [ ]) : { }
}

// 单个 提醒
const one = async (params: ONE, pk: string): Promise<ONE> => {
    let res = await net.get('notific', userPina().jwt, params, pk)
    console.log('单个 =', res)
    if (res) {
...
const creat = async (data: ONE): Promise<boolean> => {
    let res = await net.pos('notific', userPina().jwt, data)
...
const canceied = async (uids: number|string[]): Promise<boolean> => {
    let res = await net.put('notific', userPina().jwt, {
